# /Zakiev_Marat_dz_9/zakiev_marat_lesson_9_1_troubletest.py
# zakiev_marat_lesson_9_1_troubletest
# генерируется случайный цвет нарушающий порядок - для теста
...
# -------------------------------------------------------------------------------------------
#
from random import choice  # для генерации ошибки
...
class TrafficLight:
    ...
    def running(self):
        ...
        trouble = 2  # на каком цикле будет случайная ошибка
        max_count = 7  # для досрочного окончания бесконечного цикла while True -----------------------!
        ...
        while True:
            ...
            for color, t_ in zip(self.__color, timers):
                if count >= trouble:
                    color = choice(self.__color)
                    print(f'random = {color}')
                current_mode = color
                # Задачу можно усложнить, реализовав проверку порядка режимов.
                # При его нарушении выводить соответствующее сообщение и завершать скрипт.
                if control is None:
    ...
    def schedule_running(self, work_start: int = 8, work_end: int = 21):
        ...
        while hour_now in range(work_start, work_end):
            print(f'hour_now = {hour_now}, {status}')
            for color, t_ in zip(self.__color, timers):
                current_mode = color
                print(current_mode, t_)
                sleep(t_)
